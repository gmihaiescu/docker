...
RUN mkdir -p /opt/gt-download-upload-wrapper && cd /opt/gt-download-upload-wrapper && wget --no-check-certificate https://github.com/ICGC-TCGA-PanCancer/gt-download-upload-wrapper/archive/2.0.8.tar.gz && tar zxf 2.0.8.tar.gz
...
# Test perl scripts
RUN perl -c -I /opt/gt-download-upload-wrapper/gt-download-upload-wrapper-2.0.8/lib /opt/vcf-uploader/vcf-uploader-2.0.3/gnos_upload_vcf.pl && \
    perl -c -I /opt/gt-download-upload-wrapper/gt-download-upload-wrapper-2.0.8/lib /opt/vcf-uploader/vcf-uploader-2.0.3/gnos_download_file.pl && \
    perl -c -I /opt/gt-download-upload-wrapper/gt-download-upload-wrapper-2.0.8/lib /opt/vcf-uploader/vcf-uploader-2.0.3/get_donors_by_elastic_search.pl && \
    perl -c -I /opt/gt-download-upload-wrapper/gt-download-upload-wrapper-2.0.8/lib /opt/vcf-uploader/vcf-uploader-2.0.3/synapse_upload_vcf.pl
